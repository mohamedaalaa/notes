        ...
        body: local
            ? FutureBuilder(
                ...
                builder: (context, snapshot) => snapshot.connectionState ==
                    ...
                    : Consumer<GetUsers>(
                        builder: (context, getUsers, ch) => getUsers
                                .users.isEmpty
                            ? const Center(child: Text('no users yet'))
                            : ListView.builder(
                                itemCount: getUsers.users.length,
                                itemBuilder: (context, i) => ListTile(
                                      title: Text(getUsers.users[i].username),
                                    ))),
              )
            : Obx(() => (noteController.isLoading.value)
                ? const Center(
                    child: CircularProgressIndicator(),
                  )
                : (noteController.isLoading.value == false &&
                        noteController.usersList.isEmpty)
                    ? const Center(
                        ...
                        itemBuilder: (context, i) {
                          return ListTile(
                            leading: CircleAvatar(
                              radius: 30,
                              child:
                                  noteController.usersList[i].imageAsBase64 ==
                                          ""
                                      ? const Icon(Icons.person)
                                      : ClipRect(
                                          child: Image.memory(
                                          base64.decode(noteController
                                              .usersList[i].imageAsBase64),
                                          fit: BoxFit.cover,
                                        )),
                            ),
