  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title:const Text('Notes'),
        actions: [
          IconButton(icon: Icon(Icons.add),onPressed: ()=>Navigator.of(context).push(MaterialPageRoute(builder: (context)=>const UsersList())),)
        ],
      ),
      body: SizedBox(
        height: height,
        ...
        child: isSwitched?
        FutureBuilder(
          future: Provider.of<GetUsers>(context,listen: false).fetchAndSetNotes(),
          builder: (context, snapshot) => snapshot.connectionState ==
            ...
            child: CircularProgressIndicator(),
              ...
              :
          Consumer<GetUsers>(builder: (context,getNotes,ch)=>getNotes.notes.isEmpty?const Center(child: Text('no notes yet')):
          ListView.builder(itemCount: getNotes.notes.length,itemBuilder: (context,i)=>ListTile(title: Text(getNotes.users[i].username),))
          ),
        ):
        Obx(()=>
